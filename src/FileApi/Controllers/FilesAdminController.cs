...
namespace FileApi.Controllers
{
    ...
    public class FilesAdminController : ControllerBase
    {
        ...
        private readonly IFilesAdminService _filesAdminService;
        ...
        public FilesAdminController(
            IFilesAdminService filesAdminService,
            IWebHostEnvironment hostEnvironment)
        {
            _filesAdminService = filesAdminService;
        ...
        public async Task<IActionResult> DownloadAsync([Required] string? subidFile)
        {
            ...
            var check = await _filesAdminService.GetByIdAsync(subidFile);
        ...
        {
            var result = await _filesAdminService.Delete(id);
            return Ok(result);
        }

        [Route("create-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> CreateImage([FromForm] List<IFormFile> filesadd, string adminId, int width = 100, int height = 100)
        {
            ...
            {
                await _filesAdminService.InsertAsync(listEntity, adminId);
        ...
        [Route("update-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> UpdateImage([FromForm] List<IFormFile> filesadd, string adminId, int width = 100, int height = 100)
        {
            if (filesadd == null || filesadd.Count == 0)
            ...
            if (listEntity.Count() > 0)
            {
                await _filesAdminService.UpdateAsync(listEntity, adminId);
                foreach (var item in listEntity)
                {
                    var tem = new HouseWarehouseStore.Data.Entities.File();
                    tem.FileName = item.FileName;
        ...
        [HttpDelete]
        [Route("delete-files")]
        public async Task<IActionResult> DeleteImage(string adminId)
        {
            var result = false;
            if (adminId != null)
            {
                var check = await _filesAdminService.GetByIdAsync(adminId);
        ...
        {
            var files = await _filesAdminService.GetFilesAdmin(take);
            return Ok(files);
        }
        ...
        {
            var check = await _filesAdminService.GetByNameAsync(name);
