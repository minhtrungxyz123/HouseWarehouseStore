...
namespace FileApi.Controllers
{
    [Route("files-comment")]
    [ApiController]
    public class FilesCommentController : ControllerBase
    {
        #region Fields

        private readonly IWebHostEnvironment _hostingEnvironment;
        private readonly IFilesCommentService _filesCommentService;

        #endregion Fields

        #region Ctor

        public FilesCommentController(
            IFilesCommentService filesCommentService,
            IWebHostEnvironment hostEnvironment)
        {
            _filesCommentService = filesCommentService;
        ...
        public async Task<IActionResult> DownloadAsync([Required] string? subidFile)
        {
            ...
            var check = await _filesCommentService.GetByIdAsync(subidFile);
        ...
        {
            var result = await _filesCommentService.Delete(id);
            return Ok(result);
        }

        [Route("create-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> CreateImage([FromForm] List<IFormFile> filesadd, string commentId, int width = 200, int height = 200)
        {
            ...
            {
                await _filesCommentService.InsertAsync(listEntity, commentId);
                foreach (var item in listEntity)
                {
                    var tem = new HouseWarehouseStore.Data.Entities.File();
                    tem.FileName = item.FileName;
                    tem.Path = item.Path;
                    tem.CommentId = commentId;
                    listRes.Add(tem);
                }
            }
            return Ok(new
        ...
        [Route("update-image")]
        [HttpPost, DisableRequestSizeLimit]
        public async Task<IActionResult> UpdateImage([FromForm] List<IFormFile> filesadd, string commentId, int width = 200, int height = 200)
        {
            if (filesadd == null || filesadd.Count == 0)
            ...
            if (listEntity.Count() > 0)
            {
                await _filesCommentService.UpdateAsync(listEntity, commentId);
                foreach (var item in listEntity)
                {
                    var tem = new HouseWarehouseStore.Data.Entities.File();
                    tem.FileName = item.FileName;
        ...
        [HttpDelete]
        [Route("delete-files")]
        public async Task<IActionResult> DeleteImage(string commentId)
        {
            var result = false;
            if (commentId != null)
            {
                var check = await _filesCommentService.GetByIdAsync(commentId);
        ...
        [HttpGet("comment/{take}")]
        public async Task<IActionResult> GetLatestProducts(int take)
        {
            var files = await _filesCommentService.GetFilesComment(take);
            return Ok(files);
        }
        ...
        {
            var check = await _filesCommentService.GetByNameAsync(name);
