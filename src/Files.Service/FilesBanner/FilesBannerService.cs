...
{
    public class FilesBannerService : IFilesBannerService
    {
        #region Fields

        private readonly HouseWarehouseStoreDbContext _context;

        public FilesBannerService(HouseWarehouseStoreDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<long> InsertAsync(IList<HouseWarehouseStore.Data.Entities.File> entities, string bannerId)
        {
            if (bannerId is null)
            {
                throw new ArgumentNullException(nameof(bannerId));
            }
        ...
        public async Task<long> UpdateAsync(IList<HouseWarehouseStore.Data.Entities.File> entities, string bannerId)
        {
            if (bannerId is null)
            {
                throw new ArgumentNullException(nameof(bannerId));
            ...
            var itemFiles = await _context.Files.FirstOrDefaultAsync(x => x.BannerId.Equals(bannerId));
            var list = new List<HouseWarehouseStore.Data.Entities.File>();
