...
{
    public class FilesArticleService : IFilesArticleService
    {
        #region Fields

        private readonly HouseWarehouseStoreDbContext _context;

        public FilesArticleService(HouseWarehouseStoreDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<long> InsertAsync(IList<HouseWarehouseStore.Data.Entities.File> entities, string articleId)
        {
        ...
        public async Task<long> UpdateAsync(IList<HouseWarehouseStore.Data.Entities.File> entities, string articleId)
        {
            if (articleId is null)
            {
                throw new ArgumentNullException(nameof(articleId));
            ...
            var itemFiles = await _context.Files.FirstOrDefaultAsync(x => x.ArticleId.Equals(articleId));
            var list = new List<HouseWarehouseStore.Data.Entities.File>();
            foreach (var file in entities)
            {
                ...
                file.ArticleId = itemFiles.ArticleId;
            }
        ...
        public async Task<int> Delete(string id)
        {
            ...
            var item = await _context.Files.FirstOrDefaultAsync(x => x.ArticleId.Equals(id));
        ...
        public Task<HouseWarehouseStore.Data.Entities.File> GetByNameAsync(string id)
        {
            if (id is null)
            {
                throw new ArgumentNullException(nameof(id));
